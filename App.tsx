...
function App() {
  ...
  const updateScoreSheet = useCallback(async (updatedSheet: ScoreSheet) => {
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        responseMessage = bodyText || `Server rejected the score (${resp.status}).`;
      ...
      responseMessage = err instanceof Error ? err.message : 'Failed to persist score.';
    }
    ...
    return { ok: persisted, message: responseMessage };
  }, [currentUser]);
            ...
            if (currentUser?.id) {
              ...
              setAppData(prevData => {
                ...
                const existingScoreIndex = prevData.scores.findIndex(
                  s => String(s.sampleId) === normalizedSampleId && String(s.qGraderId) === headJudgeScorerId
                );
  ...
  return (
    <div className="min-h-screen bg-background font-sans text-text-dark">
      <Routes>
        ...
        />
        <Route 
          path="/qgrader-dashboard/bulk-import"
          element={
            currentUser ? (
              <QGraderDashboard
                currentUser={currentUser}
