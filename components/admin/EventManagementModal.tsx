...
const EventManagementModal: React.FC<EventManagementModalProps> = ({ isOpen, onClose, event, appData, onUpdate }) => {
    ...
    useEffect(() => {
        const fetchSamplesCount = async () => {
            if (event) {
                try {
                    const response = await fetch(`/api/cupping-events/${event.id}`);
                    const eventData = await response.json();

                    if (response.ok && eventData.samples) {
                        // Update the state with the count of samples fetched from the API
                        setSamples([{ id: 'count', count: eventData.samples.length }]);
                    } else {
                        console.error('Failed to fetch samples:', eventData.message);
                        setSamples([]);
                    }
                } catch (error) {
                ...
                setSamples([]); // Reset samples if event is null
            }
        };
