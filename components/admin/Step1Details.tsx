...
const Step1Details: React.FC<Step1DetailsProps> = ({ data, onUpdate }) => {
    ...
    const handleAddTag = () => {
        ...
        if (typeof newTag !== 'string') {
            alert('Unsupported tag type. Please enter a valid string.');
            return;
        }
        if (!(data.tags || []).every(tag => typeof tag.tag === 'string')) {
            alert('Invalid tag detected. Please ensure all tags are strings.');
            return;
        }
        if (!(data.tags || []).some(existingTag => existingTag.tag === newTag)) {
            onUpdate({
                ...data,
                tags: [...(data.tags || []), { id: `${Date.now()}`, tag: newTag }],
            });
        }
        setTagInput('');
    };
    ...
    const handleRemoveTag = (tagToRemove: string) => {
        onUpdate({
            ...data,
            tags: (data.tags || []).filter(existingTag => existingTag.tag !== tagToRemove),
        });
    };

    const renderTags = () => {
        return (data.tags || []).filter(tag => typeof tag.tag === 'string').map(tag => (
            <span key={tag.id} className="flex items-center bg-blue-100 text-blue-800 text-sm font-medium px-2.5 py-0.5 rounded-full">
                {tag.tag}
                <button onClick={() => handleRemoveTag(tag.tag)} className="ml-1.5 text-blue-500 hover:text-blue-700 focus:outline-none">
                    <X size={14} />
                </button>
            </span>
        ));
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {suggestedTags.map(tag => (
                                        <Button 
                                            ...
                                            size="sm"
                                            variant="secondary"
                                            disabled={(data.tags || []).some(existingTag => existingTag.tag === tag)}
                                            onClick={() => {
                                                if (!(data.tags || []).some(existingTag => existingTag.tag === tag)) {
                                                    onUpdate({
                                                        ...data,
                                                        tags: [...(data.tags || []), { id: `${Date.now()}`, tag }],
                                                    });
                                                }
                                            }}
