    ...
    const handleFinalSubmit = async () => {
        try {
            ...
            for (const sample of samples) {
                ...
                // Paper-based samples may have no linked farmer if offline name is provided
                if (sampleType === 'PAPER_BASED_OFFLINE') {
                    const hasLinkedFarmer = !!String(sample.farmerId || '').trim();
                    const hasOfflineName = !!String((sample as any).offlineFarmerName || '').trim();
                    if (!hasLinkedFarmer && !hasOfflineName) {
                        alert('Paper-based sample must have either a linked farmer or an offline farmer name.');
                        return;
                    }
                } else {
                    // Non-paper samples require linked farmer ID
            ...
            const newEvent = {
                ...
                samples: samples.map(sample => ({
                    ...sample,
                    farmerId: sample.farmerId ? String(sample.farmerId).trim() : null,
                    region: sample.region?.trim(),
                    // blindCode will be generated by the server based on event name
                    sampleType: sample.sampleType || 'PROXY_SUBMISSION',
                    offlineFarmerName: (sample as any).offlineFarmerName || null,
                    offlineFarmerTag: (sample as any).offlineFarmerTag || null,
                    offlineSubmissionRef: (sample as any).offlineSubmissionRef || null,
                })),
            };
            ...
            try {
                ...
                const clientSamples = (newEvent.samples || []).map(s => ({
                    ...s,
                    // keep farmerId as string for client-side state, but preserve null for offline samples
                    farmerId: s.farmerId === null || s.farmerId === undefined ? null : String(s.farmerId),
                }));
