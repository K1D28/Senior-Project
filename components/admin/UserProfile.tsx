...
const UserProfile: React.FC<UserProfileProps> = ({ user, activityLog, onBack, onUpdateUser, onUpdateStatus }) => {
  ...
  const isChanged = user && (name !== user.name || phone !== (user.phone || '') || JSON.stringify(roles.sort()) !== JSON.stringify(user.roles.sort()));

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold">User Profile: {user?.name || 'Unknown User'}</h2>
